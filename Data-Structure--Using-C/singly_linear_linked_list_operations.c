...
//start of function prototypes
	...
	void Print_Prime(NODE);					//parameters: (head)
	int Is_Prime(int);					//parameters: (int)
...
int Is_Prime(int iVal) {
	int iCount=0;

	for(iCount=2;iCount<=(iVal/2);iCount++) {
		if((iVal%iCount)==0) {
			return 0;
		}
	}
